...
include_once(dirname(__FILE__) . "/data_manager.php");
	...
	else
	{
		...
		if(!$error)
		{
			...
			if(empty($savedraft) && isset($send))
			{
				...
				if($is_file) 
				{
					$saved = iil_C_AppendFromFile($conn, "Unsent", $message);
				}
				else 
				{
					$saved = iil_C_Append($conn, "Unsent", $message);
				}
			...
			if($sent)
			{
				...
				if($is_file) 
				{
					$saved = iil_C_AppendFromFile($conn, "Sent", $message);
				}
				else 
				{
					$saved = iil_C_Append($conn, "Sent", $message);
				}
				...
				if($temp_message != false)
				{
					...
					$UID2ID = iil_C_UID2ID($conn, "Unsent", $tmp_uid);

					echo "<p>Removing temporary message in Unsent folder . . . "; flush();
					
					if(iil_C_Delete($conn, "Unsent", $UID2ID) > 0)	// Delete message from Unsent
					{
						$return = iil_C_Expunge($conn, "Unsent");		// Cleanup removed message
						echo "Success";
					}
					else
					{
				...
				include(dirname(__FILE__) . "/antispam_update.php");
			...
			else	// Message Did not Send
			{
				if($savedraft)	// Save in drafts folder
				{
					...
					if($is_file)
					{
						$saved = iil_C_AppendFromFile($conn, "Drafts", $message);
					}
					else 
					{
						$saved = iil_C_Append($conn, "Drafts", $message);
					}
...
if((isset($replyto)) || (isset($forward)) || (isset($draft)) || (isset($unsent)))
{
	...
	{		
        include_once(dirname(__FILE__) . "/mime.php");

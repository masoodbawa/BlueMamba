...
include("inc/BlueMamba/mime.php");
include("inc/BlueMamba/cache.php");
	...
	if(($my_prefs["showNav"]) && (isset($sort_field)) && (isset($sort_order)))
	{
		...
		include("inc/BlueMamba/main.php");
		$read_cache = false;
	...
	elseif( empty($header->replyto) && substr_count($header->to, "@") == 1 )
	{
		...
		else
		{
			...
			include_once("inc/BlueMamba/data_manager.php");
	...
	// Show toolbar
	include("inc/BlueMamba/read_message_tools.php");
	...
	<table width="100%" bgcolor="<?=$my_colors["main_hilite"]?>" border="0" cellspacing="1" cellpadding="1">
		<tr>
			<td>
				...
				<table width="100%" height="25" border="0" cellspacing="0" cellpadding="5" bgcolor="<?=$my_colors["main_head_bg"] ?>">
					<tr>
						<td valign="middle" class="tblheader">
							<b> &nbsp; <?php echo encodeUTFSafeHTML($header->subject); ?></b>
						</td>
						<td align="right" valign="middle" class="mainToolBar">
							<a href="<?php echo "view.php?user=$user&folder=$folder_url&id=$id&printer_friendly=1"; ?>" target="_blank" class="mainHeadingSmall">Print</a>
							<?php 	
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="5" bgcolor="<?=$my_colors["main_hilite"]?>">
					<tr bgcolor="<?=$my_colors["main_bg"]?>">
						<td valign="top">
							&nbsp; <b>Date:</b> &nbsp; <?php echo encodeUTFSafeHTML(date($DATE_FORMAT, strtotime($header->date))); ?>
							&nbsp; &nbsp; &nbsp; 
							&nbsp; <b>Size:</b> &nbsp; <?php echo ShowBytes($header->size); ?>
						</td>
					</tr>
					<tr bgcolor="<?=$my_colors["main_bg"]?>">
						<td valign="top">
							&nbsp; <b>From:</b> &nbsp; <?php echo LangDecodeAddressList($header->from, $CHARSET, $user); ?>
						</td>
					</tr>
					<tr bgcolor="<?=$my_colors["main_bg"]?>">
						<td valign="top">
							&nbsp; <b>To:</b> &nbsp; <?php echo LangDecodeAddressList($header->to, $CHARSET, $user); ?>
						</td>
					</tr>
					<?php
					...
					if(!empty($header->cc))
					{
						...
						<tr bgcolor="<?=$my_colors["main_bg"]?>">
							<td valign="top">
								&nbsp; <b>CC:</b> &nbsp; <?php echo LangDecodeAddressList($header->cc,  $CHARSET, $user); ?>
							</td>
						</tr>
						<?php
					}
					...
					if(!empty($header->replyto))
					{
						...
						<tr bgcolor="<?=$my_colors["main_bg"]?>">
							<td valign="top">
								&nbsp; <b>Reply To:</b> &nbsp; <?php echo LangDecodeAddressList($header->replyto,  $CHARSET, $user); ?>
							</td>
						</tr>
						<?php
					}
									...
									<tr><td colspan='7'></td></tr>
										...
										for($i = 1; $i <= $num_parts; $i++)
										{
											...
											//{
												...
												?>
													<td valign="middle">
														<a href="<?php echo $href; ?>" target="_blank"><img src="<?php echo $THEME . $icons_a[$type]; ?>" border="0"></a>
													</td>
													<td valign="middle" class="small">
														<a href="<?php echo $href; ?>" target="_blank"><?php
							...
							<table width="90%" align="left" border="0" cellpadding="5">
								<tr>
									<td>
										...
										include("inc/BlueMamba/read_message_handler.php");	

...
include("inc/BlueMamba/filters.php");
		...
		<td>
			<?php
					...
					<table border="0" cellspacing="0" cellpadding="5" bgcolor="<?=$my_colors["main_hilite"]?>" width="100%">
						<tr>
							<td bgcolor="<?=$my_colors["main_head_bg"] ?>">
								&nbsp; <span class="tblheader">Filters</span>&nbsp;&nbsp;
						...
						<table border="0" cellspacing="0" cellpadding="5" bgcolor="<?=$my_colors["main_hilite"]?>" width="100%">
							...
							while(list($key, $val) = each($types))
							{
								...
								if($val["type"] != $prev_type)
								{
									?>
									<tr>
										<td colspan="2">&nbsp; <b><?php echo $filter_types[$val["type"]]; ?></b></td>
									</tr>
									<?php
									$prev_type = $val["type"];
								}
								...
								<tr bgcolor="<?=$my_colors["main_bg"]?>">
									<td valign="middle">&nbsp; <a href="filters.php?user=<?=$user?>&edit=<?php echo $val["id"]; ?>"><?php echo $val["syntax"]; ?></a></td>
									<td valign="middle">&nbsp; <a href="filters.php?user=<?=$user?>&edit=<?php echo $val["id"]; ?>"><?php echo $val["moveto"]; ?></a></td>
								</tr>
								<?php
							}
							?>
				...
				if($edit > 0)
				{
					...
					?>
					<form method="post" action="<?php echo "filters.php?user=".$user; ?>" style="display:inline">
								...
								<td bgcolor="<?=$my_colors["main_bg"]?>" align="center">
									...
									<table>
										<tr>
											...
											<td colspan="3">
												<input type="text" name="edit_syntax" class="textbox" value="<?php echo $val["syntax"]; ?>" size="60">
											</td>
										</tr>
										<tr>
									...
									<input type="submit" class="button" name="save" value="Save Filter">
									<input type="submit" class="button" name="delete" value="Delete Filter">
						...
						<table border="0" cellspacing="0" cellpadding="5" bgcolor="<?=$my_colors["main_hilite"]?>" width="100%">
							...
							<tr>
								<td bgcolor="<?=$my_colors["main_bg"]?>" align="center">
										...
										<tr>
											...
											<td>
												...
												<?php 
													while(list($key, $box_name) = each($mailboxes))
													{
														if((issysfolder($box_name) == false) || ($box_name == "Trash"))
														{
									...
									<input type="submit" class="button" name="add" value="Add Filter">
